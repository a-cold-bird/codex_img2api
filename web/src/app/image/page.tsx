...
import { fetchAccounts, generateImage, type Account, type DrawModel } from "@/lib/api";
import {
...
function formatAvailableCount(accounts: Account[]) {
  const available = accounts.filter((account) => account.status === "正常").length;
  return `${available}/${accounts.length}`;
}
...
export default function ImagePage() {
  const didLoadUpstreamRef = useRef(false);
  const [imagePrompt, setImagePrompt] = useState("");
  const [imageCount, setImageCount] = useState("1");
  const [imageModel, setImageModel] = useState<DrawModel>("gpt-draw-1024x1536");
  const [conversations, setConversations] = useState<ImageConversation[]>([]);
  const [selectedConversationId, setSelectedConversationId] = useState<string | null>(null);
  const [isLoadingHistory, setIsLoadingHistory] = useState(true);
  const [isGenerating, setIsGenerating] = useState(false);
  const [availableUpstream, setAvailableUpstream] = useState("加载中");
  ...
  const loadUpstreamCount = useCallback(async () => {
    try {
      const data = await fetchAccounts();
      setAvailableUpstream(formatAvailableCount(data.items));
    } catch {
      setAvailableUpstream((prev) => (prev === "加载中" ? "—" : prev));
    }
  }, []);

  useEffect(() => {
    if (didLoadUpstreamRef.current) {
      return;
    }
    didLoadUpstreamRef.current = true;

    const syncUpstreamCount = async () => {
      await loadUpstreamCount();
    };

    const handleFocus = () => {
      void syncUpstreamCount();
    };
                ...
                <div className="absolute inset-x-0 bottom-0 flex items-center justify-between bg-gradient-to-t from-white via-white/95 to-transparent px-4 pb-4 pt-10 sm:px-6">
                  <div className="flex items-center gap-3">
                    <div className="rounded-full bg-stone-100 px-3 py-2 text-xs font-medium text-stone-600">
                      可用上游 {availableUpstream}
                    </div>
                    <Select value={imageModel} onValueChange={(value) => setImageModel(value as DrawModel)}>
